%S2_reconstruction
% 1)	missed markers (half of data): 
%	- Right Trochanter (X,Y,Z) 
%	- Right Thigh (X,Y,Z) 
%
% 2-3-4)	missed markers (half of data):
%	- Right Thigh (X,Y,Z)
%
...
%% trial 1
% 2 markers missing, so i just have 2 markers as a references
% see the code just for this

%% trial 2

filename = 'Subject 2 trial 2.xlsx';
...
filename = 'Subject 2 trial 3.xlsx';
T = readtable(filename);
...
filename = 'Subject 2 trial 4.xlsx';
T = readtable(filename);
...
%% trial 5
filename = 'Subject 2 trial 5.xlsx';
T = readtable(filename);
...
% Replace the missing values with reconstructed ones
T_filled.LeftThigh_X(missing_idx) = P4_recon(missing_idx, 1);
T_filled.LeftThigh_Y(missing_idx) = P4_recon(missing_idx, 2);
T_filled.LeftThigh_Z(missing_idx) = P4_recon(missing_idx, 3);
