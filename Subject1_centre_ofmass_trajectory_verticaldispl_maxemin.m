...
% ====== PARAMETRI ======
fileName1 = 'SubjectBis 1 trial 1.xlsx';  % Nome del primo file Excel
fileName2 = 'SubjectBis 1 trial 2.xlsx';  % Nome del secondo file Excel
fileName3 = 'SubjectBis 1 trial 3.xlsx';  % Nome del terzo file Excel
fileName4 = 'SubjectBis 1 trial 4.xlsx';  % Nome del quarto file Excel
fileName5 = 'SubjectBis 1 trial 5.xlsx';  % Nome del quinto file Excel
...
% ====== VISUALIZZAZIONE DELLE TRAIETTORIE NELLO SPAZIO 3D ======
figure;
hold on;

% Traccia la traiettoria del primo marcatore (primo trial)
plot3(X1, Y1, Z1, '-o', 'LineWidth', 2, 'DisplayName', 'Trial 1');  % Primo marcatore
% Traccia la traiettoria del secondo marcatore (secondo trial)
plot3(X2, Y2, Z2, '-x', 'LineWidth', 2, 'DisplayName', 'Trial 2');  % Secondo marcatore
% Traccia la traiettoria del terzo marcatore (terzo trial)
plot3(X3, Y3, Z3, '-s', 'LineWidth', 2, 'DisplayName', 'Trial 3');  % Terzo marcatore
% Traccia la traiettoria del quarto marcatore (quarto trial)
plot3(X4, Y4, Z4, '-^', 'LineWidth', 2, 'DisplayName', 'Trial 4');  % Quarto marcatore
% Traccia la traiettoria del quinto marcatore (quinto trial)
plot3(X5, Y5, Z5, '-d', 'LineWidth', 2, 'DisplayName', 'Trial 5');  % Quinto marcatore

% Aggiungi il titolo e le etichette per gli assi
title('Confronto delle Traiettorie tra i Vari Trial');
xlabel('Z');
ylabel('Y');
zlabel('X');
grid on;
axis equal;  % Imposta le stesse proporzioni per gli assi

% Imposta limiti per gli assi in modo dinamico
zlim([min([X1; X2; X3; X4; X5])-1, max([X1; X2; X3; X4; X5])+1]);  % Limiti asse X
ylim([min([Y1; Y2; Y3; Y4; Y5])-1, max([Y1; Y2; Y3; Y4; Y5])+1]);  % Limiti asse Y
xlim([min([Z1; Z2; Z3; Z4; Z5])-1, max([Z1; Z2; Z3; Z4; Z5])+1]);  % Limiti asse Z

% Aggiungi una legenda per distinguere le traiettorie
legend('show');

% Impostazioni della vista
view(3);  % Imposta la vista in 3D
rotate3d on;  % Permette la rotazione interattiva del grafico
%%
figure;
